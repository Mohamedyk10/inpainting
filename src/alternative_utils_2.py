...
"""def determine_closest_patch(target_region, patches : dict, contour_patch, p):
    ...
    return keys_patches[min_index]""" #old one
...
def determine_closest_patch(target_region, patches: dict, contour_patch, p, search_radius):
    ...
    #list of keys that are within the search radius
    filtered_keys = []
    for q_key in patches.keys():
        q_i, q_j = q_key
        # Use a simple square check
    ...
    #Build the matrix using ONLY the filtered keys
